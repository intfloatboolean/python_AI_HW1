...
with col2:
    api_key = st.text_input("API ключ openweathermap.org",
                            type="password", # Для сокрытия от нежелательных глаз
...
if mode == "📊 Анализ исторических данных (загрузка файла CSV)":
    ...
    if uploaded_file is not None:
        with st.spinner("Анализ данных..."):
            ...
            # Описательная статистика и сезонные характеристики
            ...
            # Нормальный диапазон температур берем из файла
...
# =============================================================================
# РЕЖИМ 2 Получение текущей температуры через API
if mode == "🌤️ Текущая температура через API openweathermap.org" or uploaded_file is None:
...
if api_key:
    ...
    if data.get("cod") == 401:
        ...
        source = "загруженного файла" if mode.startswith("📊") and uploaded_file is not None else "встроенных данных"
